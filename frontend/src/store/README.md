...
- **`transcript` holds MANY conversations, keyed by exploration.** Until
  ...
  - **How a stream addresses its own conversation:** every action takes an
    ...
    about what the reader is looking at (clearing the chat, spotlighting a
    citation) stay unkeyed and unchanged, while the streaming paths in
  ...
  - **Discoveries are the one thing that cannot simply follow.** The workspace
    ...
    opened.
  - `running` (stream ids, not a flag: several explorations can be answering
    at once) drives the rail's still-working dots and tells the
...
- **`transcript`** — written by the teacher's stream dispatches; read by the
  ...
  **A graph load keeps the whole conversation** (v7.21.0). It used to keep the
  `chat` and drop the lecture, and the slice's two halves genuinely had
  different owners: the chat is the *user's* — they asked those questions, and
  loading another graph says nothing about being finished with the answers —
  while a lecture belonged to the *graph*, narrating the neighborhood you built
  with beats pointing at that graph's nodes. There is no second half any more,
  so nothing is dropped. Home (`workspaceCleared`) still empties it; a restore
  replaces it. Safe for the reason the chat always was — citations degrade: an
  `[n]` whose paper is no longer loaded renders greyed and inert instead of
  silently highlighting nothing (`teacher/transcript/README.md`), a bubble
  stops being clickable when none of its papers are loaded, and beats go
  through the same check. What none of that does is *say* which graph a
  now-inert turn came from, which is the turn's provenance line's job. A load
  seeded from a citation used to carry a `fromChat` flag that opened the new
  seed's detail panel on arrival; both flag and panel went in v7.11.0 (the
  panel covered the graph the click was waiting for), so a graph load now says
  nothing about what is selected.

  **A lecture is a turn** (`chatBeatAdded` → `ChatMsg.beats`), and the turn
  records which assistant the router picked (`turnRouted` →
  `ChatMsg.routedTo`) so the transcript can offer the other. Beats reuse the
  turn's own `sourceRefs`, which chat messages already carry.

  **Every turn also records the graph it was answered over** (`turnGraphSet` →
  `ChatMsg.graph`: the seed's id and title, plus how many papers were in
  scope). Dispatched at turn start, while that graph is still the one on
  screen. It exists because a conversation outlives its graph: the transcript
  survives a graph load, so one conversation can hold turns about several
  graphs, and everything that already handled that handled it *negatively* —
  a stale `[n]` greys out, a bubble stops being clickable. None of that says
  which graph a now-inert turn came from. The count is each turn's own rather
  than the graph's node count, because an answer's scope
  (`selectGroundingNodes`) and a lecture's (`selectLectureNodes`) differ.

  It was a **slot** until v7.21.0: `lecture` (its beats, or null) plus
  `lectureShown` so hiding it didn't throw the beats away, `lectureSources` for
  the one `[Sn]` index every beat shared, and `lectureStarted` / `beatAdded` /
  `lectureShownAgain` / `lectureHidden` / `lectureDropped` around them. Before
  v7.17.0 it was four slots, one per mode button. All of it existed because a
  lecture came from a button rather than a message, and *replace* semantics are
  right for a button — pressing Lecture twice means "show me the lecture" —
  and wrong for a message, where a second request is a second reply the reader
  can scroll back to.

  **Restore still reads three eras** (`restoredLectureTurn` in `workspace.ts`)
  and folds whatever it finds into `chat` as a turn: a v7.17.0-era save's
  single `lecture`; a v6-era per-mode `lectures` cache, out of which it picks
  the one that was on screen (else the first played, in `LEGACY_MODE_ORDER`)
  and drops the rest; and an ancient flat `beats` array. Folding rather than
  ignoring is the point — the destination slot is gone, so a restore that did
  nothing would silently discard the reader's lecture. Note that
  `lectureSources` holds **two shapes** across those eras — a marker index in
  the later one, a map of mode → marker index in the earlier — discriminated
  by which lecture field the save carries, because renaming the field would
  have left every existing save's sources unreadable. Saves predating
  structured library citations have none; those markers degrade to raw text.
- **Two grounding scopes, differing on one question** (`scopedNodes` +
  ...
  reader filters to 2024+, then asks for a lecture — which is exactly the kind
  of case that would have gone unexplained.
...
- **`library`** — the uploaded sources, `loadLibrary`-thunk-fetched: the
  ...
  the one initial fetch. The *scope choices* stay panel-local (tracked by
  exclusion, so a new source is searchable by default); only the list itself
  is shared. A 🎓 lecture-scope picker read `transcript` the same way until
  v7.21.0, when a lecture became ordinary conversation history and there was
...
## A selector that exists to keep the frontend and backend agreeing
...
There were two. `selectSatelliteCount` counted the papers scoping would
exclude, for a sentence the Lecture section showed about them, and it
deliberately re-derived the **same predicate the backend uses** (is this
joined to the seed by an edge?) rather than reusing `_origin`, the layout hint
`clusterForce` computes for orbiting satellites — two independent notions of
"satellite" being exactly how a UI note drifts out of sync with the behaviour
it describes. The section went in v7.21.0 and the selector went with it; that
reasoning is recorded here because it applies again the moment anything wants
to say what a lecture will leave out.
