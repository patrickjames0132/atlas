...
| [03](03_targeted-rerating/README.md) | The high-endorsement nots are calibration misses only where the paper is big in its own right: blind stronger raters relabel the 2,220-citer survey, not the three one-big-citer artifacts | 30-row blind sheet: 4 feature-nominated nots among 26 uniform decoys; 2–3 deeper raters, told nothing of the nomination; per-rater labels, never merged | Sheet built (deterministic, reproducible); recruitment fell through — parked pending raters | Targeted re-rating is bottlenecked on qualified-rater time; the anchor's 135 nots are only 123 distinct papers (10 citers sampled under 2–3 seeds) — row-level counts double-count; and the design's "shuffle by the same hash" would have sunk all 4 nominees to the sheet's bottom (draw hashes aren't uniform over the drawn set) — presentation uses a salted hash instead |
