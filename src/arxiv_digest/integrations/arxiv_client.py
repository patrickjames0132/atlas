...
def _date_clause(year_from: int | None, year_to: int | None) -> str | None:
...
def _category_clause(categories: list[str] | None) -> str | None:
...
def search_arxiv(
    ...
    if id_match:
        ...
        category_clause = _category_clause(categories)
        if category_clause:
            parts.append(category_clause)
        date_clause = _date_clause(year_from, year_to)
        if date_clause:
