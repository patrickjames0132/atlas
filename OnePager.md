...
- [ ] **Lecture turns never reach the researcher as history** — v7.21.0
...
- [ ] **Make the provenance line a control, not a caption** — **superseded
...
- [ ] **Threads — one thread, one graph** — the conversation model gets a
      ...
      **Decided.**
      ...
      - **A thread owns its transcript and its graph state.** Opening another
        ...
        *grows*, so every paper a turn cites is on that thread's graph by
        construction. **Inspecting a citation never switches threads**: an
        `[n]` opens the paper's detail, and *Explore in a new thread* is an
        action on the detail, not a side effect of the click.
