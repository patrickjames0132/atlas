...
Instructions for Codex when working in `~/PyCharmProjects/atlas`. Read this first;
...
> repo generically as **"the developer."** Codex: silently read that as the
...
0. **Branch off `main` for a major feature** — before starting a substantial
   ...
   **`problem/<short-name>`** — see "Research & analysis work".) **Working
   ...
   doc-only changes** — updating READMEs, other markdown, `AGENTS.md`,
...
**Browser automation (Codex in Chrome) is ask-first.** Driving the developer's
...
(`.Codex/skills/research/SKILL.md` — deliberately absent from `main`). For
...
`research` with `main`, verify `.Codex/skills/research/` survived — restore
it with `git checkout research -- .Codex/skills/research` if the merge
dropped it.
...
- **Commit:** stage files **explicitly**. End
  the message with a `Co-Authored-By` trailer naming **the Codex model actually
