...
def test_figure_above_caption_is_found(tmp_path):
    ...
    mined = floats.extract_floats(pdf)
    assert len(mined) == 1
    entry = mined[0]
...
def test_prose_reference_is_not_a_caption(tmp_path):
    ...
    assert floats.extract_floats(pdf) == []
...
def test_max_floats_cap(tmp_path, monkeypatch):
    monkeypatch.setattr(config.pdf, "max_floats", 2)
...
def test_render_float_returns_png(tmp_path):
    ...
    entry = floats.extract_floats(pdf)[0]
