...
def _raw_paper(**over):
    raw = {
        ...
        "githubStars": 15000,
        "linkedModels": [
            {"id": f"org/model-{i}", "likes": 100 - i, "downloads": 1000 * i,
...
def test_code_links_normalizes_full_paper(monkeypatch):
    monkeypatch.setattr(hf, "_fetch_paper", lambda aid: _raw_paper())
    ...
    assert out["github"] == {
        "url": "https://github.com/tensorflow/tensor2tensor", "stars": 15000}
    assert len(out["models"]) == 5  # capped at _MAX_ITEMS
    assert out["models"][0] == {
        "id": "org/model-0", "url": "https://huggingface.co/org/model-0",
        "likes": 100, "downloads": 0, "pipeline_tag": "text-generation"}
...
def test_code_links_handles_sparse_paper(monkeypatch):
    ...
    monkeypatch.setattr(hf, "_fetch_paper", lambda aid: {
        "id": "2301.00001", "upvotes": None,
        "linkedModels": [{"likes": 3}, "junk"], "linkedSpaces": None,
    })
...
def test_code_links_rejects_non_github_repo_url(monkeypatch):
    monkeypatch.setattr(hf, "_fetch_paper", lambda aid: _raw_paper(
        githubRepo="https://evil.example/repo"))
