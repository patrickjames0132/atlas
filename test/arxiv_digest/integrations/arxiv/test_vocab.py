"""arxiv.vocab: the bundled arXiv category taxonomy (groups + valid_codes).
...
from arxiv_digest.integrations.arxiv import vocab
...
def test_valid_codes_covers_exactly_the_codes_in_groups():
    from_groups = {
        category["code"] for group in vocab.groups() for category in group["categories"]
    }
    assert vocab.valid_codes() == from_groups
